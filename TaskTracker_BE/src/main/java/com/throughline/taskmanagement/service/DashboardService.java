...
public interface DashboardService {
    ...
    /** The Executive's Dashboard view: every CRITICAL-severity task org-wide plus every task an
     *  Executive/Super Admin personally assigned, any depth (see TaskRepository.
     *  findBySeverityOrAssignedByRoleIn). */
    Page<TaskListResponse> getExecutiveTasks(Long viewerId, Pageable pageable);

    /** The Director Dashboard's "Critical & CEO-assigned" panel — the department-scoped equivalent of
     *  getExecutiveTasks: every HIGH/CRITICAL-severity task within this Director's own department, plus
     *  every task in it an Executive/Super Admin personally assigned. */
